...
@section('head')
    ...
    <style>
        ...
        .table td, .table thead th,.footer, .sidebar .nav .nav-item .nav-link .menu-title,p {
            font-size: 14px;
        }
    </style>
    <script>
...
@section('content')
    ...
    <div class="card-box">
        ...
        <h3>Import Group Of Translations By Key:</h3>
        <div class="row">
            <div class="col-md-8">
                <form class="form-import" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postImport') ?>" data-remote="true" role="form">
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-sm-6">
                                <select name="replace" class="select2 form-control">
            ...
            <div class="col-md-4">
                <form class="form-find" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postFind') ?>" data-remote="true" role="form" data-confirm="Are you sure you want to scan you app folder? All found translation keys will be added to the database.">
                    <div class="form-group">
                        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                        <button type="submit" class="btn btn-block btn-info" data-disable-with="Searching.." >Find translations in files</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <form role="form" method="POST" action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAddGroup') ?>">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
            <div class="form-group">
                <h3>Choose a group to display the group translations:</h3>
                <select style="height: 40px;" name="group" id="group" class="form-control group-select">
                    <?php foreach($groups as $key => $value): ?>
                    <option value="<?php echo $key ?>"<?php echo $key == $group ? ' selected':'' ?>><?php echo str_well($value) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </form>
        ...
        <fieldset>
            <h3>Publish all translations</h3>
